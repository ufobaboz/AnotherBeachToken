...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (caller.role === 'admin' && targetRole !== 'operator') {
    console.log(JSON.stringify({ event: 'soft-delete-profile', status: 409, error: 'forbidden_role_target', caller_id: caller.userId, target_id: targetId, target_role: targetRole, latency_ms: Date.now() - startedAt }));
    return jsonResponse(errorBody('forbidden_role_target', 'Solo super_admin puo\' cancellare un admin.'), 409);
  }
  if (caller.role === 'super_admin' && targetRole !== 'operator' && targetRole !== 'admin') {
    console.log(JSON.stringify({ event: 'soft-delete-profile', status: 409, error: 'forbidden_role_target', caller_id: caller.userId, target_id: targetId, target_role: targetRole, latency_ms: Date.now() - startedAt }));
    return jsonResponse(errorBody('forbidden_role_target', 'Target non cancellabile.'), 409);
  }
