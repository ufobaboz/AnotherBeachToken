...
> {
  ...
  if (!authHeader.toLowerCase().startsWith('bearer ')) {
    console.log(JSON.stringify({ event: 'reset-password', status: 401, error: 'missing_auth' }));
  ...
  if (userResp.error || !userResp.data?.user) {
    console.log(JSON.stringify({ event: 'reset-password', status: 401, error: 'invalid_token' }));
  ...
  if (profileResp.error) {
    console.error(JSON.stringify({ event: 'reset-password', status: 500, error: 'unknown_error', message: profileResp.error.message }));
    return { ok: false, resp: jsonResponse(errorBody('unknown_error', profileResp.error.message), 500) };
  }
  if (!profileResp.data) {
    console.log(JSON.stringify({ event: 'reset-password', status: 403, error: 'profile_not_found' }));
    return { ok: false, resp: jsonResponse(errorBody('profile_not_found', 'Profilo non trovato.'), 403) };
  }
  if (profileResp.data.deleted_at !== null) {
    console.log(JSON.stringify({ event: 'reset-password', status: 403, error: 'profile_deleted' }));
  ...
  if (role !== 'admin' && role !== 'super_admin') {
    console.log(JSON.stringify({ event: 'reset-password', status: 403, error: 'forbidden_role' }));
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (targetResp.error) {
    console.error(JSON.stringify({ event: 'reset-password', status: 500, error: 'unknown_error', message: targetResp.error.message, caller_id: caller.userId, target_id: targetId, latency_ms: Date.now() - startedAt }));
  ...
  if (!targetResp.data || targetResp.data.deleted_at !== null) {
    console.log(JSON.stringify({ event: 'reset-password', status: 404, error: 'target_not_found', caller_id: caller.userId, target_id: targetId, latency_ms: Date.now() - startedAt }));
  ...
  if (caller.role === 'admin' && targetRole !== 'operator') {
    console.log(JSON.stringify({ event: 'reset-password', status: 409, error: 'forbidden_role_target', caller_id: caller.userId, target_id: targetId, target_role: targetRole, latency_ms: Date.now() - startedAt }));
    return jsonResponse(errorBody('forbidden_role_target', 'Solo super_admin puo\' resettare la password di un admin.'), 409);
  }
  if (caller.role === 'super_admin' && targetRole !== 'operator' && targetRole !== 'admin') {
    console.log(JSON.stringify({ event: 'reset-password', status: 409, error: 'forbidden_role_target', caller_id: caller.userId, target_id: targetId, target_role: targetRole, latency_ms: Date.now() - startedAt }));
    return jsonResponse(errorBody('forbidden_role_target', 'Target non valido.'), 409);
  }
  ...
  if (updResp.error) {
    ...
    if (msg.includes('password') && (msg.includes('weak') || msg.includes('short') || msg.includes('characters'))) {
      console.log(JSON.stringify({ event: 'reset-password', status: 422, error: 'weak_password', caller_id: caller.userId, target_id: targetId, latency_ms: Date.now() - startedAt }));
      return jsonResponse(errorBody('weak_password', 'Password troppo debole.'), 422);
    }
    console.error(JSON.stringify({ event: 'reset-password', status: 500, error: 'update_failed', message: updResp.error.message, caller_id: caller.userId, target_id: targetId, latency_ms: Date.now() - startedAt }));
    return jsonResponse(errorBody('unknown_error', updResp.error.message), 500);
  }

  console.log(JSON.stringify({ event: 'reset-password', status: 200, caller_id: caller.userId, target_id: targetId, latency_ms: Date.now() - startedAt }));
